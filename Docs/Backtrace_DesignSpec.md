...
>
> **Phase 37 — Live Preverb 1.6B (performance + short-time + octave).** Cubase test: volume under control, but stalls + short/fast times broke. **Stalls/CPU:** the convolution head went 512 → **2048** (NonUniform) — the preverb already has seconds of latency so a low-latency head was wasted CPU; bigger head = far fewer small FFTs = much lower CPU for long IRs (the main stall on longer Swell Times). Kernel rebuilds are now **debounced 40 ms** on the worker (knob-drag bursts coalesce into one rebuild) and guarded by a **mutex** (no reverb-object race). `markTailDirty` only re-requests the IR when Live mode is on. (Remaining: a TIME/FEEL change still changes the reported latency, which forces a host PDC re-sync — inherent to a latency-based preverb; non-TIME params do not.) **Short-time (1/8–1/16):** a short window was dominated by the reverb's 35 ms pre-delay + a fixed 30 ms fade → near-silent IR. Now the kernel scales to the window — pre-delay shrinks (`applyReverb` gained `predelayScale`), envelope is **proportional** (fade-in M/8, out M/12), and a MICRO factor adds density + faster decay so even 1/16 makes a real short reversed smear (verified: 1/16 noise output RMS 0.09, audible — not dry/silent). **Octave:** was dead in Live (pitch never reached the live path). Now the reversed kernel is **resampled to pitch-shift** it (a genuinely higher/lower preverb), landing kept aligned, only when nonzero — verified it audibly changes the kernel (maxDiff 0.06) at all times. **UI:** the live status shows the musical range — **PREVERB** (1/2+), **TIGHT** (1/4–1/8), **MICRO** (1/16–1/32) — so short modes read as intentional, not broken. DBG log per rebuild (time/M/reverb/octave/micro). RenderTest 52/52 (×3 stable), AU passes, DSP safety ALL SAFE, Capture mode untouched.
>
> **Phase 38 — Live Preverb freeze fix (latency oscillation + RT safety).** Cubase froze (transport stuck) after ~a minute. Root cause: a **latency-reporting oscillation**. `prepareToPlay` reported latency one way (`convLatency()+preverbLength`) while `setIR` computed the real dry-delay from `conv.getLatency()` — which is **unreliable** (returns 0 before the async kernel load, the real value after). When the two disagreed, the editor timer kept calling `setLatencySamples`+`updateHostDisplay`, Cubase re-inited → called `prepareToPlay` → disagreed again → an endless host PDC re-sync loop that locks the audio engine. Fixes: (1) `LivePreverb::convLatency()` returns the **fixed** NonUniform head (deterministic) instead of `getLatency()`; (2) `prepareToPlay` reports latency with the **identical** formula the editor timer uses (`isReady() ? getLatencySamples() : 0`) — single source of truth, so the host can't be caught in a loop. Defensive RT-safety added: `LivePreverb::process` clamps the convolution to the prepared block size (a larger host block would overflow `wetBuf`), and the live IR length is **capped at 5 s** (longer convolutions are CPU-heavy and a >5 s preverb = >5 s latency anyway). RenderTest updated to publish latency via `pushLiveLatencyIfChanged` (mirrors the editor timer); 52/52 (×3 stable), AU passes, DSP safety ALL SAFE.
>
> **Phase 39 — Live Preverb: smooth synthetic kernel (root-cause fix for harsh/blasting sound).** The live preverb sounded harsh/metallic/"8-bit" on real sources (e.g. acoustic guitar), and gain was an unwinnable fight. Root cause: it built its kernel by REVERSING an algorithmic reverb — whose modal ringing (harsh), Ghost-Shimmer octave pitch-shift (chiptune), and resonances (blast on tonal input) all get spotlighted by the reverse build-up. Fix: `rebuildLiveIR` now SYNTHESISES the reverse-reverb kernel directly as **dense decorrelated velvet-noise** (random ± impulses, ~2600/s) → a smooth diffuse wash with **no modal resonance, no shimmer, no pitch artifacts** — shaped by a build-up envelope (`t^riseExp`, punchier for micro windows) and a tone filter (HPF de-mud + LPF warmth). Reverb TYPE + Tone knob now just set the kernel COLOUR (dark↔bright); Octave became a brightness tilt (a diffuse wash has no pitch to shift). A non-resonant kernel is both smooth AND can't blast — verified: full-wet sustained peak dropped 0.56 → **0.29**, default 25% Mix 0.41 < 0.5 dry, still leads in, 1/16 audible, octave changes the kernel. Default reverb reverted **Ghost Shimmer → Velvet Hall** (warm). Convolution/latency/stability path unchanged; RenderTest 52/52 (×2 stable), AU passes, DSP safety ALL SAFE, Capture mode untouched. (Sound quality itself is ear-judged — this removes the two known root causes; a pitched-preverb layer, if wanted, is a separate future addition.)
>
> **Phase 40 — Live Preverb: high-quality kernel + transparent ceiling.** Quality pass on the synthetic kernel so it sounds expensive, not generic. The flat-brightness velvet wash became a **lush, evolving reverse-reverb swell**: (1) **frequency-dependent decay** — a position-varying lowpass opens from DARK far-tail → BRIGHT landing (`fc = exp(lerp(lnDark, lnBright, t^0.7))`), so the swell blooms open like a real hall tail reversed (the single biggest expensive-vs-generic factor); (2) **exponential build-up** (`env = exp((t-1)·rate)`) — the natural reverb decay reversed, giving a stronger, more musical rise into the landing (measured 12× vs ~7×); (3) **mono-safe stereo** — the kernel's SIDE is high-passed (240 Hz) so lows stay centred and only highs spread, no hollow/phasey mono collapse; (4) denser velvet (~4500/s). Reverb TYPE + Tone knob set landing brightness; Octave tilts it. Also: the live output **ceiling is now transparent below 0.8** (soft-clip only near clipping, `0.8 + 0.2·tanh`) instead of an unconditional `tanh` that coloured clean signal. Audited the live process path: buffer/prepared guards, denormal-safe, zipper-free gain ramp, valid circular dry-delay, exact FFT convolution, no hard cuts. Full-wet peak 0.25 (non-resonant → can't blast), RenderTest 52/52 (×2 stable), AU passes, DSP safety ALL SAFE, Capture mode untouched. Sound is ear-judged; density/tone/build-up are one-line tunables for feedback.
>
